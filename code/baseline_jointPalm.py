...
from fast_utils import cosine_similarity, load_vectors, spearman, getAverageWordRep, read_sets, getAverageWordRep2
import pickle
...
if __name__ == "__main__":
	...
	if len(sys.argv) < 8:
		print "USAGE: python baselline_word2vec.py <PATH TO TASK> <PATH TO WORDVECTORS> <PATH TO SVMS> <PATH TO CLUSTERS> <PATH TO REL> <PATH TO EXPANSIONSCACHE> <PATH TO NORMAL VECTORS"
		sys.exit()
	...
	for i in xrange(len(questions)):
		...
		if word1 in disambiguatedWords:
			...
			new = word1 + "_" + label
			if new in vectors:
				vec11 = vectors[word1 + "_" + label]
				vec12 = normalVectors[word1]
				vec1 = getAverageWordRep2([vec11, vec12])
				w1 = True
